...
def calculate_winrate():
    try:
        ...
        for psychologie, donnees in donnees_par_psychologie.items():
            if psychologie not in resultats_par_psychologie:
                for doc in donnees:
                    ...
                    # calcul
                    recuperationFiltreDeBase = str(doc.get(filtreDeBase))
                    recuperationFiltreAnnexe = str(doc.get(filtreAnnexe))

                    print(recuperationFiltreDeBase)
                    print(recuperationFiltreAnnexe)

                    # assignation de la valeur au tableau
                    if recuperationFiltreDeBase is not None:
                        resultats_par_psychologie[recuperationFiltreDeBase] = recuperationFiltreAnnexe

                print(resultats_par_psychologie)

            typeEnregistrement = f"{filtreDeBase}_{filtreAnnexe}"

            resultats_par_psychologie["typeEnregistrement"] = typeEnregistrement
            print("5")
        print(resultats_par_psychologie)
        ...
        result = collection_temporaire.update_one({'typeEnregistrement': typeEnregistrement}, {'$set': resultats_par_psychologie}, upsert=True)
        ...
        return jsonify(resultats_par_psychologie), 200
