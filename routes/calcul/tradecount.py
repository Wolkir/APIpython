...
def calculate_tradecount(data):
    ...
    if status == "open":
        last_opened_trade = collection_open.find_one({"dateAndTimeOpening": {"$regex": f"^{date_of_trade}"}}, sort=[("trade_number", -1)])
        if last_opened_trade:
            new_trade_number = last_opened_trade["trade_number"] + 1
        else:
