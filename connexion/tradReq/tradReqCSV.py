...
# Trade Blueprint
trade_blueprint_csv = Blueprint('trade_blueprint_csv', __name__)
SLOpen = {}
RROpen = {}
TPOpen = {}
...
@trade_blueprint_csv.route('/savetraderequestCSV', methods=['POST'])
def save_trade_request_csv():
    uploaded_file = request.files['file']
    if uploaded_file:
        csv_data = []
        
        column_names = uploaded_file.readline().decode('utf-8').strip().split('|')
        
        for line in uploaded_file:
            line_data = line.decode('utf-8').strip().split('|')
            
            line_dict = {column_names[i]: value for i, value in enumerate(line_data)}
    ...
    closure_position = csv_data.get('closurePosition')
...
app.register_blueprint(trade_blueprint_csv, url_prefix='/api')
